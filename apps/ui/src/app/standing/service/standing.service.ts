...
import { ErgastDriverResult, IDriverResult, IDriverStanding, mapper } from '@f2020/data';
...
export class StandingService {
  ...
  getDriverResult(seasonId: string, driverId: string): Observable<IDriverResult> {
    return this.service.get<ErgastDriverResult>(`${seasonId}/drivers/${driverId}/results.json`, result => result.MRData.RaceTable.Races).pipe(
      map(raceResults => {
        const results = mapper.driverResults(raceResults);
